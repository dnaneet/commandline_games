...
def strength_factor_evaluate(t1r, t2r, n):
    if(np.max(t1r, t2r)/np.min(t1r, t2r)>=6): 
        return 6
    else:
        return nSamples/2000        
...
next_round = np.array('') #Empty array has been created.  this will store 
                          #names of teams that make it to the next round
                          #This will be updated in the code below.

#pdb.set_trace()

nTotalRounds = int(np.log(len(df)*2)/np.log(2))
matches_in_this_round = len(df)

round = 1;
j=0; #team number for subsequent rounds
#while(round <= len(df) - 1): #nTotalRounds
while(round <= nTotalRounds):
    for match in range(matches_in_this_round):    
        if(round <= len(df)-1):                
            ...
            possession = np.vstack((t1_possession, t2_possession))
            plt.imshow(possession, cmap='jet')
            fileName = team_1_name +'_v_' + team_2_name + '.pdf'
